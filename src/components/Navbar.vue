...
<template>
  <header class="bg-[#050a16] border-b border-cyan-500/25 px-3 py-1 flex flex-col gap-1 select-none z-40 relative shadow-[0_4px_20px_rgba(0,0,0,0.6)]">
    ...
    <div class="flex items-center justify-between h-9">
          ...
          >
            ...
            <div class="space-y-0.5 mt-1">
              <button
                ...
                class="w-full flex items-center justify-between px-2 py-1 rounded text-xs font-mono transition-colors text-left hover:bg-cyan-500/20 text-slate-200 cursor-pointer"
                :class="{ 'bg-cyan-950/60 text-cyan-300 font-bold border border-cyan-500/30': screen.width === res.w && screen.height === res.h }"
              >
                <div class="flex items-center gap-1">
                  <Check v-if="screen.width === res.w && screen.height === res.h" class="w-3 h-3 text-cyan-400" />
                  <span v-else class="w-3" />
                  <span>{{ res.w }} × {{ res.h }}</span>
                </div>
                <span class="text-[9px] px-1 py-0.2 rounded bg-slate-800/80 text-slate-400 font-mono">
                  {{ res.tag }}
                </span>
              </button>
            </div>
          ...
          >
            ...
            <div class="space-y-0.5 mt-1">
              <button
                ...
                class="w-full flex items-start gap-1.5 p-1.5 rounded text-xs font-mono transition-colors text-left hover:bg-cyan-500/20 text-slate-200 cursor-pointer"
              >
                <div class="w-1.5 h-1.5 rounded-full bg-cyan-400 mt-1 shrink-0" />
                <div>
      ...
      <div class="flex items-center gap-1.5">
        ...
        <button
          ...
          class="p-1.5 rounded-lg text-xs font-mono font-bold transition-all cursor-pointer border"
          :class="isStreaming 
        ...
        <button
          ...
          class="p-1.5 rounded-lg bg-slate-900 border border-slate-700 hover:border-cyan-500 hover:text-cyan-300 text-slate-300 transition-all cursor-pointer"
        ...
        <button
          ...
          class="p-1.5 rounded-lg bg-amber-950/40 border border-amber-500/50 hover:border-amber-400 hover:bg-amber-950 text-amber-300 transition-all cursor-pointer shadow-[0_0_8px_rgba(245,158,11,0.15)]"
        ...
        <button
          ...
          class="p-1.5 rounded-lg bg-slate-900 border border-slate-700 hover:border-cyan-500 hover:text-cyan-300 text-slate-300 transition-all cursor-pointer relative"
        ...
        <button
          ...
          class="flex items-center gap-1 px-2 py-1 rounded bg-slate-900 border border-slate-700 hover:border-cyan-400 text-xs font-mono text-slate-200 transition-all cursor-pointer"
    ...
    <div class="flex items-center justify-between bg-slate-950/90 border border-cyan-500/20 px-2 py-0.5 rounded-lg text-xs font-mono select-none">
      ...
      <div class="flex items-center gap-1">
        ...
        <div class="flex items-center bg-slate-900 p-0.5 rounded-md border border-slate-800">
          <button
            @click="emit('undo')"
            :disabled="!canUndo"
            class="p-1 rounded hover:bg-slate-800 text-slate-300 disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer transition-colors"
            title="撤销 (Ctrl+Z)"
          >
        ...
        <div class="flex items-center bg-slate-900 p-0.5 rounded-md border border-slate-800 gap-1">
          <button
            ...
            :class="showGrid ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/40' : 'text-slate-400 hover:text-white'"
            title="显示/隐藏网格底图"
          >
          ...
          <button
            ...
            class="p-1 rounded cursor-pointer transition-all"
            :class="snapToGrid ? 'bg-cyan-950 text-cyan-300 border border-cyan-500/50' : 'text-slate-400 hover:text-white'"
          ...
          <button
            ...
            class="p-1 rounded cursor-pointer transition-all"
            :class="orthogonalLock ? 'bg-cyan-950 text-cyan-300 border border-cyan-500/50' : 'text-slate-400 hover:text-white'"
        ...
        <div class="flex items-center bg-slate-900 p-0.5 rounded-md border border-slate-800 gap-0.5">
          <button
            @click="emit('align', 'left')"
            :disabled="selectedIds.length < 2"
            class="p-1 rounded hover:bg-slate-800 text-slate-300 disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
            title="左对齐"
          >
        ...
        <div class="flex items-center bg-slate-900 p-0.5 rounded-md border border-slate-800 gap-0.5">
          <button
            @click="emit('group')"
            :disabled="selectedIds.length < 2"
            class="p-1 rounded hover:bg-slate-800 text-cyan-300 disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
            title="组合为群组 (Ctrl+G)"
          >
          ...
          <button
            ...
            class="p-1 rounded hover:bg-slate-800 text-amber-300 disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
          ...
          <button
            ...
            class="p-1 rounded hover:bg-slate-800 text-emerald-300 disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
      ...
      <!-- Right: Zoom Controls -->
      <div class="flex items-center gap-1">
        <div class="flex items-center bg-slate-900 p-0.5 rounded-md border border-slate-800 text-xs font-mono">
          <button
            @click="emit('update:zoom', Math.max(0.1, Number((zoom - 0.1).toFixed(2))))"
            class="p-1 rounded hover:bg-slate-800 text-slate-300 cursor-pointer transition-colors"
            title="缩小"
          >
            <ZoomOut class="w-3.5 h-3.5" />
          </button>
          <span class="text-[11px] font-mono font-bold text-cyan-300 px-1 w-10 text-center">
            {{ Math.round(zoom * 100) }}%
          </span>
          <button
