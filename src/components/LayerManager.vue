...
<template>
  <aside class="w-68 h-full bg-[#060a15] border-r border-cyan-500/20 flex flex-col select-none z-30 shadow-xl overflow-hidden font-sans">
    <!-- Header -->
    <div class="p-3 border-b border-slate-800 bg-[#040812]">
      <div class="flex items-center justify-between mb-2">
        ...
        <span class="text-[10px] font-mono px-1.5 py-0.5 rounded bg-slate-900 border border-slate-700 text-slate-400">
      ...
      <!-- Search Filter -->
      <div class="relative">
        <Search class="w-3 h-3 absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-500" />
        <input
          v-model="filterKeyword"
          type="text"
          placeholder="搜索图层名称/类型..."
          class="w-full bg-slate-950 border border-slate-800 focus:border-cyan-400 rounded-lg pl-7 pr-2 py-1 text-[11px] text-slate-200 outline-hidden font-mono"
        />
      </div>
    </div>
    ...
    <div class="flex-1 overflow-y-auto p-1.5 space-y-1 custom-scrollbar">
      <div
        ...
        class="text-center py-10 text-xs font-mono text-slate-600"
      >
        暂无图层或未匹配到元件
      </div>
      ...
      <div
        ...
        :class="activeSelectedIds.includes(comp.id) 
          ? 'bg-cyan-950/70 border-cyan-400 text-cyan-200 shadow-xs' 
          : 'bg-slate-950/40 border-slate-900 text-slate-400 hover:bg-slate-900/80 hover:text-slate-200 hover:border-slate-800'"
      >
        ...
        <div class="flex items-center gap-1.5 min-w-0 flex-1">
          ...
          <button
            ...
            class="p-0.5 rounded hover:bg-slate-800 text-slate-500 hover:text-slate-200 transition-colors"
            :title="comp.visible === false ? '显示图层' : '隐藏图层'"
          >
            <EyeOff v-if="comp.visible === false" class="w-3.5 h-3.5 text-slate-600" />
          ...
          >
            ...
            <Unlock v-else class="w-3.5 h-3.5 opacity-40 group-hover:opacity-100" />
          </button>
